...
brainstorm_recipes_task = Task(
    description=(
        "Using the dietary requirements given as a Python list of one dictionary from the previous task, interpret each key: 'dietary', 'cuisine', 'time', 'ingredients_to_use', 'ingredients_to_avoid'. "
        "Your ONLY job is to generate a Python list of 3-5 dictionaries, each with keys: 'name', 'description', and 'tags'. "
        f"Do NOT generate any meal idea whose name matches any of these previous meal names: {read_meal_names()}. "
        "Do NOT include any commentary, explanation, confirmation, or status update. "
        "Your output MUST be a valid Python list of dictionaries, and nothing else."
    ),
